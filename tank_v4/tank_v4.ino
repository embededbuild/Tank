...
// ================================================================
// nRF24 - VSPI (Shared with SD card)
// ================================================================
#define NRF24_CE   4
#define NRF24_CSN  5
SPIClass vspi(VSPI);
...
void cleanupPicoClient() {
  if (pPicoClient) {
    if (pPicoClient->isConnected()) {
      pPicoClient->disconnect();
    }
    NimBLEDevice::deleteClient(pPicoClient);
    pPicoClient = nullptr;
    pCommandCharacteristic = nullptr;
    picoConnected = false;
  }
...
void connectToPico() {
    ...
    if (picoConnected && pPicoClient) {
        addResult("Already connected!");
        addResult("Disconnect first to");
        addResult("reconnect");
    ...
    if (!pPicoClient->connect(picoDevice)) {
        ...
        addResult("Try again or check");
        addResult("Pico distance");
        cleanupPicoClient();
        currentMenu = MENU_RESULT;
        drawResult();
        return;
    ...
    if (pCommandCharacteristic->canWrite()) {
        if (pCommandCharacteristic->writeValue(authToSend.c_str(), authToSend.length())) {
            addResult("Auth sent to Pico");
            picoConnected = true;
        } else {
            addResult("Auth send failed");
            cleanupPicoClient();
            currentMenu = MENU_RESULT;
            drawResult();
            return;
...
void sendCommandToPico(String command) {
    ...
    if (!pCommandCharacteristic->writeValue(command.c_str(), command.length())) {
        Serial.println("[CMD] Write failed!");
        addResult("Send failed!");
        return;
    }
...
void runWifiScanAll(bool openOnly) {
  ...
  if (n == 0) {
    ...
    for (int i = 0; i < n && shown < 40; i++) {
      ...
      // Save to scan string
      lastWifiScan += (isOpen ? "[OPEN] " : "[SEC]  ") + ssid + " (" + String(WiFi.RSSI(i)) + "dBm)\n";
    }
  }
  WiFi.scanDelete();
  WiFi.mode(WIFI_OFF);
...
void runBLEScan(bool trackersOnly) {
  ...
  MyScanCallbacks* callbacks = new MyScanCallbacks();
  ...
  lastBLEScan = "=== BLE Scan [" + getTimestamp() + "] ===\n";
  lastBLEScan += trackersOnly ? "Mode: Trackers Only\n" : "Mode: All Devices\n";
  lastBLEScan += "Found: " + String(bleDevCount) + "\n\n";
  ...
  for (int i = 0; i < bleDevCount; i++) {
    ...
    lastBLEScan += (d.isTracker ? "[TRACKER] " : "[DEVICE] ") + display + " (" + String(d.rssi) + "dBm)\n";
  }
...
void runNRF24Scan() {
  ...
  if (!nrfReady) {
    addResult("nRF24 not ready!");
    currentMenu = MENU_RESULT;
    drawResult();
    return;
  }
  
  showMessage("Scanning RF24...", "Checking 125 ch", "");
  memset(chActivity, 0, sizeof(chActivity));
  totalPackets = 0;
  activeChannels = 0;
  ...
  for (int ch = 0; ch < NUM_CHANNELS; ch++) {
    ...
    radio.stopListening();
    if (chActivity[ch] > 2) activeChannels++;
  }
  
  lastRF24Data = "=== RF24 Spectrum [" + getTimestamp() + "] ===\n";
  lastRF24Data += "Active Channels: " + String(activeChannels) + "/125\n";
  lastRF24Data += "Total Packets: " + String(totalPackets) + "\n\n";
  
  addResult("Active ch: " + String(activeChannels) + "/125");
  addResult("Packets: " + String(totalPackets));
  addResult("Top channels:");
  
  for (int i = 0; i < 5 && i < NUM_CHANNELS; i++) {
    int maxCh = 0;
    int maxVal = 0;
    for (int ch = 0; ch < NUM_CHANNELS; ch++) {
      if (chActivity[ch] > maxVal) {
        maxVal = chActivity[ch];
        maxCh = ch;
      }
    }
    if (maxVal > 0) {
      ...
      addResult(String(buf));
      lastRF24Data += String(buf) + "\n";
      chActivity[maxCh] = 0;
    }
  }
...
void runNRF24Capture() {
  ...
  showMessage("Capturing RF24...", "Scanning channels", "");
  captureCount = 0;
  memset(captureBuffer, 0, sizeof(captureBuffer));
  
  for (int ch = 0; ch < NUM_CHANNELS && captureCount < MAX_CAPTURED_PACKETS; ch++) {
    radio.setChannel(ch);
    radio.startListening();
    unsigned long start = millis();
    while (millis() - start < CAPTURE_DURATION_MS && captureCount < MAX_CAPTURED_PACKETS) {
      if (radio.available()) {
        CapturedPacket& pkt = captureBuffer[captureCount];
        pkt.length = radio.getDynamicPayloadSize();
        if (pkt.length > 32) pkt.length = 32;
        radio.read(pkt.data, pkt.length);
        pkt.channel = ch;
  ...
  addResult("Captured: " + String(captureCount));
  addResult("packets");
  addResult("From " + String(NUM_CHANNELS) + " channels");
  
  if (captureCount > 0) {
    char buf[32];
    snprintf(buf, sizeof(buf), "Pkt1: CH%d L%d", captureBuffer[0].channel, captureBuffer[0].length);
    addResult(String(buf));
  }
...
void runNRF24Replay() {
  ...
  if (captureCount == 0) {
    addResult("No packets captured!");
    addResult("Capture packets first");
    currentMenu = MENU_RESULT;
    drawResult();
    return;
  }
  
  showMessage("Replaying packets...", (String(captureCount) + " packets").c_str(), "");
  int replayed = 0;
  
  for (int i = 0; i < captureCount; i++) {
    radio.stopListening();
    radio.setChannel(captureBuffer[i].channel);
    radio.write(captureBuffer[i].data, captureBuffer[i].length);
    replayed++;
    delay(5);
  }
  
  addResult("Replayed: " + String(replayed));
  addResult("packets sent");
...
void saveRF24Data() {
  if (lastRF24Data.length() == 0) {
    clearResults("SD: SAVE RF24");
    addResult("No RF24 scan data!");
    addResult("Run spectrum scan first");
    currentMenu = MENU_RESULT;
    drawResult();
    return;
  }
  saveToSD("/tank/rf24/rf24_" + getTimestamp() + ".txt", lastRF24Data);
}
