...
enum ResultContext { CTX_NONE, CTX_WIFI, CTX_BLE, CTX_RF24, CTX_PROBE };
...
// ================================================================
// drawResult — shows scroll hint bar + SEL=Save / BACK=Menu hint
// ================================================================
void drawResult() {
  oledClear();
  u8g2.setFont(u8g2_font_5x7_tf);

  // Reserve bottom 8px for the hint bar
  int maxLines = 8;
  int lineH   = 7;
  int startY  = 7;
  ...
  // Scrollbar (fits within top 56px)
  if (resultLineCount > maxLines) {
    int barH = 56 * maxLines / resultLineCount;
    int barY = 56 * menuScroll / resultLineCount;
    u8g2.drawFrame(124, 0, 4, 56);
...
void drawSplash() {
  ...
  for (int i = 0; i < 8; i++) drawGlitchEffect(8 - i, 8 - i);
...
void runWifiScanAll(bool openOnly) {
  ...
  resultOriginMenu = MENU_WIFI;      // <-- FIX: remember origin
  resultContext    = CTX_WIFI;       // <-- FIX: enable save from result
  ...
  for (int i = 0; i < n && wifiNetworkCount < MAX_WIFI_NETWORKS; i++) {
    ...
    lastWifiScanData += (isOpen ? "[OPEN] " : "[SEC] ") + ssid +
  ...
  // Re-assert pullups after WiFi hardware releases GPIO mux
  pinMode(BTN_BACK,   INPUT_PULLUP);
  pinMode(BTN_UP,     INPUT_PULLUP);
  pinMode(BTN_DOWN,   INPUT_PULLUP);
  pinMode(BTN_SELECT, INPUT_PULLUP);
  delay(100);  // let pullups stabilize
  ...
  switch (currentMenu) {
    ...
    case MENU_RESULT:
      switch (resultContext) {
        case CTX_WIFI:  saveWifiScan(); break;
